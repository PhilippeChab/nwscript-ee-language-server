...
describe("Installed standalone LSP server", function () {
  ...
  it("indexes comma-containing paths and continues past individual file failures", async () => {
    ...
    expect(messages[1].syntaxIndex?.globalDeclarations.some((declaration) => declaration.identifier === "Helper")).to.equal(true);
    const client = await start();
    await client.ready();
      ...
      writeFileSync(
        ...
        process.once("message", paths => {
          const timer = setInterval(() => {
            ...
            clearInterval(timer);
            for (const filePath of paths) process.send({ filePath, syntaxIndex: {
