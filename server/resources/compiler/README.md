...
The compiler remains a subprocess. Each validation uses temporary copies of the saved target and indexed includes, so compiler lookup agrees with the language server and existing source-side build artifacts are preserved. `-s` prevents writing build artifacts and `--no-require-entry-point` enables syntax and semantic validation of standalone includes. The include-depth limit is at least 64, increased to accommodate larger indexed dependency sets.

Upstream 2.3.1 reports the first error per compilation and does not provide the old compiler's warning coverage. It rejects empty includes, script basenames longer than 16 characters, and some syntax accepted by `nwnsc`. Game and user directories must exist; commas in additional directory paths are unsupported. Saving an empty target clears its diagnostics without invoking the compiler.
