...
export default class ServerManger {
  ...
  constructor(connection: Connection, params: InitializeParams) {
    ...
    this.standardLibrary = new StandardLibrary(this.workspaceFilesSystem, this.parser, (message) => this.logger.error(message));

    this.liveDocumentsManager.listen(this.connection);
  }

  public async initialize() {
    await this.parser.loadGrammar();
          ...
          await this.indexFiles(paths.slice(index * size, (index + 1) * size), (message) => {
            ...
            if (message.syntaxIndex) {
              ...
              if (this.workspaceFilesSystem.getRootForUri(uri) && !this.liveDocumentsManager.get(uri) && existsSync(message.filePath)) {
                this.documentsCollection.addDocument(uri, message.syntaxIndex);
              }
              indexed++;
  ...
  private updateDocument(document: TextDocument) {
    ...
    try {
      this.documentsCollection.updateDocument(document, this.parser, this.workspaceFilesSystem);
    } catch (error) {
      // Register unfinished new documents; retain existing usable scopes.
      if (!this.documentsCollection.getWorkspaceDocument(document.uri)) {
        this.documentsCollection.addDocument(document.uri, { includes: [], globalDeclarations: [], structDeclarations: [] });
      }
